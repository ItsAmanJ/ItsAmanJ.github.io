...
(function initCursor() {
  ...
  const AURA_LAG = 0.12; // aura trails behind cursor
  ...
  // Smooth aura with lerp
  function animateCursor() {
    auraX += (mouseX - auraX) * AURA_LAG;
    auraY += (mouseY - auraY) * AURA_LAG;
  ...
  const interactables = 'a, button, .btn, .tag, .hobby-card, .skill-card, .gaming-card, .yt-thumb-card, .social-link, .flow-link, .nav-link, .mob-link';
    ...
    entries => {
      entries.forEach(entry => {
        ...
        function step(now) {
          ...
          el.innerHTML = val + suffix.replace(/<sup>.*<\/sup>/, '') + sup;
