/* ================================================================
   ...
   Covers:
     ...
     8. Google Slides embed helper
     9. Visitor counter (hero + footer, starts at 293)
================================================================ */
...
/* ----------------------------------------------------------------
   9.  VISITOR COUNTER
       ...
       • Tries countapi.mileshilliard.com first (shared across all browsers)
       • Falls back to localStorage (per-device) if API is unreachable
       • Counter always displays 293+ (SEED offset = 292)
       • Updates both the hero badge and the footer pill
       • Uses sessionStorage so tab refreshes don't inflate count
---------------------------------------------------------------- */
(function initViewCounter() {
  const SEED     = 292;   /* displayed count = SEED + API raw value; first hit → 293 */
  const KEY      = 'itsamanj-portfolio-views';
  const BASE_URL = 'https://countapi.mileshilliard.com/api/v1';

  /* FIX: declare el variables — were missing in original code */
  const footerEl = document.getElementById('viewCount');
  const heroEl   = document.getElementById('heroViewCount');

  /* Show loading state on footer pill */
  if (footerEl) {
